# Historical report: persistent LangGraph agent (course block 6.4)

> Preserved course evidence with results from that point in time. Current
> runtime behavior is documented in [`../agent-persistent-report.md`](../agent-persistent-report.md).
...
Сервис `postgres` в `docker-compose.yml` использует образ `postgres:16.14-alpine`, БД
