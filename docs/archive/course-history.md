...
> Historical evidence. This file preserves the original course-by-course README
> and intermediate commands/results. It is not the current production runbook.
> For the final product, use the repository root `README.md` and
...
[`../deployment.md`](../deployment.md). Локальный dev workflow остаётся
...
[`agent-naive-traces/README.md`](agent-naive-traces/README.md); реальные
...
stdout описаны в [`agent-react-scenarios.md`](agent-react-scenarios.md).
Фактические метрики, средние значения и ограничения локального запуска собраны
в [`agent-react-report.md`](agent-react-report.md); искусственные
...
[`agent-graph-report.md`](agent-graph-report.md). Baseline-файлы
